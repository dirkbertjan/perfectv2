# This scripts trains the ablation of prompt+mte in table 1.
...
# 3) For 'learning_rate', we sweep over [1e-1, 1e-2, 1e-3].
...
python run_clm.py configs/prompt_mte_ablation.json 